...
func (r *Redactor) Summary() string {
	...
	sb.WriteString(fmt.Sprintf("  - Rules Loaded:  %d\n", len(r.config.Rules)))
	sb.WriteString(fmt.Sprintf("  - Detectors:     %d\n", len(r.detectors)))
	...
	// Section 4: Unique requests and rules affected
	r.mu.Lock()
	...
	sb.WriteString("\n[Impact Summary]\n")
	sb.WriteString(fmt.Sprintf("  - Requests Affected:     %d\n", len(uniqueRequests)))
	sb.WriteString(fmt.Sprintf("  - Unique Rules Triggered: %d / %d\n", len(uniqueRules), len(r.config.Rules)))
	sb.WriteString(fmt.Sprintf("  - Unique Detections:     %d\n", len(details)))

	// Section 5: Top triggered rules
	if len(ruleHitCount) > 0 {
		sb.WriteString("\n[Top Triggered Rules]\n")
		sb.WriteString("+-------------------------------+-------+\n")
		sb.WriteString("| Rule ID                       | Hits  |\n")
		sb.WriteString("+-------------------------------+-------+\n")
		for ruleID, count := range ruleHitCount {
			displayID := ruleID
			if len(displayID) > 29 {
				displayID = displayID[:26] + "..."
			}
			sb.WriteString(fmt.Sprintf("| %-29s | %5d |\n", displayID, count))
		}
		sb.WriteString("+-------------------------------+-------+\n")
	}
	...
	for _, d := range details {
		...
		sb.WriteString(fmt.Sprintf("| %-16s | %-8s | %-29s | %-16s |\n", reqIDShort, d.DetectorType, ruleIDShort, maskedShort))
	}
	sb.WriteString("+------------------+----------+-------------------------------+------------------+\n")
